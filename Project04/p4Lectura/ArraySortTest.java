public class ArraySortTest {
    private static int[] accessCounts = new int[]{2000, 55964, 23258, 17877, 22752, 15868, 4023};
	...
	if (c <= array.length()) {
	    ...
	    System.out.println("Your access count looks good enough for extra credit!");
	    score += 3.0;
	} else if (c <= (int)(1.5*accessCounts[testNum-1])) {
	    System.out.println("Your access count looks good enough for full credit!");
	    score += 2.0;
	} else if (c <= 2*accessCounts[testNum-1]) {
	    System.out.println("Your access count is not too bad, but it could be better!");
	    score += 1.0;
	} else {
